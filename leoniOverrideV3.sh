...
# Restituisce l'IP scelto senza validazione (usata internamente)
_pick_raw() {
...
# Verifica se un IP è già usato su una LAN (da router o host)
# Uso: _ip_in_use <ip> <lan> <nome_da_escludere>
# Ritorna 0 (usato) o 1 (libero)
_ip_in_use() {
    local CHECK_IP="$1" CHECK_LAN="$2" EXCLUDE="$3"
    for OR in "${ROUTERS[@]}"; do
        [[ "$OR" == "$EXCLUDE" ]] && continue
        local EX="${RT_IP["${OR}:${CHECK_LAN}"]}"
        [[ -n "$EX" && "$EX" == "$CHECK_IP" ]] && return 0
    done
    for OH in "${HOSTS[@]}"; do
        [[ "$OH" == "$EXCLUDE" ]] && continue
        [[ "${H_LAN[$OH]}" != "$CHECK_LAN" ]] && continue
        [[ "${H_IP[$OH]}" == "$CHECK_IP" ]] && return 0
    done
    return 1
}
...
for ((i=0; i<N_ROUTERS; i++)); do
    ...
    for LAN in $RLANS; do
        ...
        # Avviso preventivo: mostra IP già assegnati su questa LAN da altri router
        for OTHER_R in "${ROUTERS[@]}"; do
            [[ "$OTHER_R" == "$RNAME" ]] && continue
            EXISTING="${RT_IP["${OTHER_R}:${LAN}"]}"
            if [[ -n "$EXISTING" ]]; then
                warn "${OTHER_R} è già su ${LAN} con IP ${EXISTING} — scegli un IP diverso!"
            fi
        done

        # Loop con ri-richiesta finché l'IP non è libero
        while true; do
            CHOSEN_IP=""
            _pick_raw "${LAN_CIDR[$LAN]}" CHOSEN_IP "  $RNAME/eth${ETH} su $LAN"
            if _ip_in_use "$CHOSEN_IP" "$LAN" "$RNAME"; then
                warn "IP ${CHOSEN_IP} già assegnato su ${LAN} — scegli un altro."
            else
                break
            fi
        done
...
if [[ "$_inet" =~ ^[sS] ]]; then
    ...
    echo ""
    info "Opzionale: IP del router ISP su cui fare default route (es. 2.3.4.25)"
...
for LAN in "${LANS[@]}"; do
    ...
    for ((j=1; j<=NH; j++)); do
        ...
        while true; do
            ...
            if _ip_in_use "$CHOSEN_IP" "$LAN" "$HNAME"; then
                warn "IP ${CHOSEN_IP} già assegnato su ${LAN} — scegli un altro."
            else
                break
            fi
...
# ── README riepilogativo ─────────────────────────────────────────
{
    ...
    for R in "${ROUTERS[@]}"; do
        ...
        [[ -n "${RT_DEFGW[$R]}" ]] && echo "  - Default route: ${RT_DEFGW[$R]}"
    done
    echo ""
    echo "### Host"
    for H in "${HOSTS[@]}"; do
...
if [[ ${#ROUTERS[@]} -gt 0 ]]; then
    ...
    for R in "${ROUTERS[@]}"; do
        ...
        [[ -n "${RT_DEFGW[$R]}" ]] && echo -e "    ${YELLOW}↳${NC} ${RT_DEFGW[$R]}"
    done
fi
